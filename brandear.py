import datetime
from ebaysdk.exception import ConnectionError
from ebaysdk.finding import Connection
from datetime import datetime, timedelta
from ebaysdk.trading import Connection as Trading
from tqdm import tqdm
import itertools
import numpy as np
import pandas as pd
from ebaysdk.shopping import Connection as Shopping
import flask
import os
from ebaysdk.trading import Connection as Trading
from ebaysdk.exception import ConnectionError


app = flask.Flask(__name__)





# YOUR_APPID = 'MathieuK-brandear-PRD-c13e2103b-cda61bfa'
# YOUR_AUTH_TOKEN = 'v^1.1#i^1#f^0#I^3#p^3#r^1#t^Ul4xMF8wOkNENDM3MTM5M0FBMjc2OTQ3RTkwMENFNjA1OTdGMDY5XzJfMSNFXjI2MA=='
# YOUR_DEVID = '3b233cb3-45cb-4f15-befe-3843b07826e9'
# YOUR_CERTID='3b233cb3-45cb-4f15-befe-3843b07826e9'

# api = Trading(appid=YOUR_APPID, devid=YOUR_DEVID, certid=YOUR_CERTID, token=YOUR_AUTH_TOKEN, config_file=None)
# response = api.execute('GetItem', {'ItemID': '234625563894', 'IncludeItemSpecifics': 'true'}).dict()

# conditions = {
#     '4': 'Poor condition, obviously used and / or has problems',
#     '5': 'Poor condition, obviously used and / or has minor problems',
#     '6': 'Fair condition, used',
#     '7': 'Great condition, slightly used',
#     '8': 'Excellent Condition, barely used',
#     '9': 'Almost New',
#     '10': 'Brand New'
# }


# title = response['Item']['Title']
# price = response['Item']['ListingDetails']['ConvertedStartPrice']['value']
# currency = response['Item']['ListingDetails']['ConvertedStartPrice']['_currencyID']
# name_list = {val['Name']: val['Value'] for val in response['Item']['ItemSpecifics']['NameValueList']}
# try:
#     item_type = name_list['Type']
# except:
#     item_type = ''
# try:
#     brand = name_list['Brand']
# except:
#     brand = ''
# try:
#     model = name_list['Product Name']
# except:
#     model = ''
# try:
#     material = name_list['Exterior Material']
# except:
#     material = ''
# try:
#     line = name_list['Line']
# except:
#     line = ''
# try:
#     stamp = name_list['Date Code/Stamp']
# except:
#     stamp = ''
# try:
#     color = name_list['Exterior Color']
# except:
#     color = ''
# main_photo = response['Item']['PictureDetails']['PictureURL'][0]
# other_photos = response['Item']['PictureDetails']['PictureURL'][1:]
# try:
#     accessories = name_list['Accessories'].split(', ')
# except:
#     accessories = ''
# original_packaging = ''
# try:
#     condition = name_list['ConditionRank']
# except:
#     condition = ''
# try:
#     condition_comments = conditions[condition]
# except:
#     condition_comments = ''
# item_id = response['Item']['ItemID']
# item_url = response['Item']['ListingDetails']['ViewItemURL']
# try:
#     serial_number = name_list['Model Number']
# except:
#     serial_number = ''



# this will make 'search' request two times with parameters
# q=drone and limit=50 for the first time and
# category_ids=20863 for the second time




# APP_ID = 'MathieuK-brandear-PRD-c13e2103b-cda61bfa'

# token_id = 'v^1.1#i^1#I^3#r^0#p^3#f^0#t^'

# def get_results(payload):
#     days = 2
#     start = datetime.today()
#     while start >= datetime.today() - timedelta(days=30):
#         end = start - timedelta(days=days)
#         try:
#             # api = Connection(appid=APP_ID, config_file=None, devid='3b233cb3-45cb-4f15-befe-3843b07826e9', certid='PRD-13e2103b8ab5-5e97-4157-a0d0-c671', token=token_id)
#             api = Connection(appid=APP_ID, config_file=None, devid='3b233cb3-45cb-4f15-befe-3843b07826e9', certid='3b233cb3-45cb-4f15-befe-3843b07826e9')
#             # response = api.execute('findItemsAdvanced', {'StartTimeFrom': start,'StartTimeTo': end}, payload)
#             response = api.execute('FindItemsIneBayStores', payload)

#             # assert(response.reply.ack == 'Success')
#             # assert(type(response.reply.timestamp) == datetime.datetime)
#             # assert(type(response.reply.searchResult.item) == list)

#             return response.dict()
#             # assert(type(item.listingInfo.endTime) == datetime.datetime)
#             # assert(type(response.dict()) == dict)

#         except ConnectionError as e:
#             print(e)
#             print(e.response.dict())
#         start = end

# payload = {
#     'storeName': 'Brandear',
#     "paginationInput": { "pageNumber": "1" }
# }


# 'Auth LOUIS VUITTON Geronimos N51994 Ebene Damier CA0064 Bum Bag'



# api.execute('findItemsIneBayStores', payload).dict()['searchResult']['item'][0]




class Brandear:
    def __init__(self):
        self.APP_ID = 'MathieuK-brandear-PRD-c13e2103b-cda61bfa'
        self.token_id = 'v^1.1#i^1#f^0#I^3#p^3#r^1#t^Ul4xMF8wOkNENDM3MTM5M0FBMjc2OTQ3RTkwMENFNjA1OTdGMDY5XzJfMSNFXjI2MA=='
        self.devid = '3b233cb3-45cb-4f15-befe-3843b07826e9'
        self.certid='3b233cb3-45cb-4f15-befe-3843b07826e9'
        self.api = Connection(appid=self.APP_ID, config_file=None, devid=self.devid, certid=self.certid)
        self.conditions = {
            '4': 'Poor condition, obviously used and / or has problems',
            '5': 'Poor condition, obviously used and / or has minor problems',
            '6': 'Fair condition, used',
            '7': 'Great condition, slightly used',
            '8': 'Excellent Condition, barely used',
            '9': 'Almost New',
            '10': 'Brand New'
        }
    def get_total_pages(self):
        request = {
            'storeName': 'Brandear',
        }
        return int(self.api.execute('findItemsIneBayStores', request).dict()['paginationOutput']['totalPages'])

    def get_items(self):
        self.items = []
        for i in tqdm(range(1, 101)):
            request = {
                "storeName": "Brandear",
                "paginationInput": { "pageNumber": f"{i}" }
            }
            page_items = self.api.execute('findItemsIneBayStores', request).dict()['searchResult']['item']
            self.items.append(page_items)

    def get_individual_item(self, data):
        rows = []
        for val in tqdm(data.item_id.values):

            api = Trading(appid=self.APP_ID, config_file=None, devid=self.devid, certid=self.certid, token=self.token_id)
            # api = Trading(appid=YOUR_APPID, config_file=None, devid=YOUR_DEVID, certid=YOUR_CERTID, token=YOUR_AUTH_TOKEN)
            response = api.execute('GetItem', {'ItemID': val, 'IncludeItemSpecifics': 'true'}).dict()

            title = response['Item']['Title']
            price = response['Item']['ListingDetails']['ConvertedStartPrice']['value']
            currency = response['Item']['ListingDetails']['ConvertedStartPrice']['_currencyID']
            name_list = {val['Name']: val['Value'] for val in response['Item']['ItemSpecifics']['NameValueList']}
            try:
                item_type = name_list['Type']
            except:
                item_type = ''
            try:
                brand = name_list['Brand']
            except:
                brand = ''
            try:
                model = name_list['Product Name']
            except:
                model = ''
            try:
                material = name_list['Exterior Material']
            except:
                material = ''
            try:
                line = name_list['Line']
            except:
                line = ''
            try:
                stamp = name_list['Date Code/Stamp']
            except:
                stamp = ''
            try:
                color = name_list['Exterior Color']
            except:
                color = ''
            main_photo = response['Item']['PictureDetails']['PictureURL'][0]
            other_photos = response['Item']['PictureDetails']['PictureURL'][1:]
            try:
                accessories = name_list['Accessories'].split(', ')
            except:
                accessories = ''
            try:
                condition = name_list['ConditionRank']
            except:
                condition = ''
            try:
                condition_comments = self.conditions[condition]
            except:
                condition_comments = ''
            item_id = response['Item']['ItemID']
            item_url = response['Item']['ListingDetails']['ViewItemURL']
            try:
                serial_number = name_list['Model Number']
            except:
                serial_number = ''
            
            row = {
                'title': title,
                'price': price,
                'currency': currency,
                'item_type': item_type,
                'brand': brand,
                'model': model,
                'material': material,
                'line': line,
                'stamp': stamp,
                'color': color,
                'serial_number': serial_number,
                'main_photo': main_photo,
                'other_photos': other_photos,
                'accessories': accessories,
                'condition': condition,
                'condition_comments': condition_comments,
                'item_id': item_id,
                'item_url': item_url
            }
            rows.append(row)

        return pd.DataFrame(rows)

    def __call__(self):
        self.total_pages = self.get_total_pages()
        self.get_items()
        return self.items


@app.route('/')
@app.route('/home')
def home():
    return "waiting"


@app.route('/items')
def get_items():
    brandear = Brandear()
    items = brandear()
    items = list(itertools.chain.from_iterable(items))

    item_id = np.array([val['itemId'] for val in items])
    item_title = np.array([val['title'] for val in items])
    item_image = np.array([val['galleryURL'] for val in items])
    item_category = np.array([val['primaryCategory']['categoryName'] for val in items])
    item_price = np.array([float(val['sellingStatus']['convertedCurrentPrice']['value']) for val in items])
    item_currency = np.array([val['sellingStatus']['convertedCurrentPrice']['_currencyId'] for val in items])
    item_url = np.array([val['viewItemURL'] for val in items])

    data = pd.DataFrame(columns=['item_id', 'item_title', 'item_image', 'item_category', 'item_price', 'item_currency', 'item_url'])
    data['item_id'] = item_id
    data['item_title'] = item_title
    data['item_image'] = item_image
    data['item_category'] = item_category
    data['item_price'] = item_price
    data['item_currency'] = item_currency
    data['item_url'] = item_url

    # Filter by price (less than 400 sgd) and by category
    data = data[data['item_price'] < 400/1.4][data['item_category'] == "Women's Bags & Handbags"]
    data.reset_index(inplace=True, drop=True)

    data = brandear.get_individual_item(data)

    # data2 = [data.columns.values.tolist()]
    # data2.extend(data.values.tolist())
    # value_range_body = {"values": data2}

    # value_range_body = {
    #     "majorDimension":"COLUMNS",
    #     "values":
    #         [data2]
    #     }

    return data.to_json(orient="records")


# APP_ID = 'MathieuK-brandear-PRD-c13e2103b-cda61bfa'
# token_id = 'v^1.1#i^1#I^3#r^0#p^3#f^0#t^'
# devid = '3b233cb3-45cb-4f15-befe-3843b07826e9'
# certid='3b233cb3-45cb-4f15-befe-3843b07826e9'
# api = Connection(appid=APP_ID, config_file=None, devid=devid, certid=certid)

# request = {
#     "storeName": "Brandear",
#     "paginationInput": { "pageNumber": "101" },
#     "itemFilter": [
#       {
#         "name": "ModTimeFrom",
#         "value": f"{(datetime.today() - timedelta(days=2)).isoformat()}",
#       }
#     ]
# }
# api.execute('findItemsIneBayStores', request).dict()['searchResult']['item']



if __name__ == "__main__":
    app.secret_key = 'ItIsASecret'
    # app.debug = True
    app.run(debug=True, host="0.0.0.0", port=int(os.environ.get("PORT", 8080)))

# if __name__ == "__main__":
#     app.secret_key = 'ItIsASecret'
#     app.debug = True
#     app.run()




                   
                   
                   



# items = Brandear()()
# items = list(itertools.chain.from_iterable(items))

# item_id = np.array([val['itemId'] for val in items])
# item_title = np.array([val['title'] for val in items])
# item_image = np.array([val['galleryURL'] for val in items])
# item_category = np.array([val['primaryCategory']['categoryName'] for val in items])
# item_price = np.array([float(val['sellingStatus']['convertedCurrentPrice']['value']) for val in items])
# item_currency = np.array([val['sellingStatus']['convertedCurrentPrice']['_currencyId'] for val in items])
# item_url = np.array([val['viewItemURL'] for val in items])

# data = pd.DataFrame(columns=['item_id', 'item_title', 'item_image', 'item_category', 'item_price', 'item_currency', 'item_url'])
# data['item_id'] = item_id
# data['item_title'] = item_title
# data['item_image'] = item_image
# data['item_category'] = item_category
# data['item_price'] = item_price
# data['item_currency'] = item_currency
# data['item_url'] = item_url

# # Filter by price (less than 400 sgd) and by category
# data = data[data['item_price'] < 400/1.4][data['item_category'] == "Women's Bags & Handbags"]
# data.reset_index(inplace=True, drop=True)

# rows = []
# for val in tqdm(data.item_id.values):

#     # api = Trading(appid=self.APP_ID, config_file=None, devid=self.devid, certid=self.certid, token=self.token_id)
#     api = Trading(appid=YOUR_APPID, config_file=None, devid=YOUR_DEVID, certid=YOUR_CERTID, token=YOUR_AUTH_TOKEN)
#     response = api.execute('GetItem', {'ItemID': val, 'IncludeItemSpecifics': 'true'}).dict()

#     title = response['Item']['Title']
#     price = response['Item']['ListingDetails']['ConvertedStartPrice']['value']
#     currency = response['Item']['ListingDetails']['ConvertedStartPrice']['_currencyID']
#     name_list = {val['Name']: val['Value'] for val in response['Item']['ItemSpecifics']['NameValueList']}
#     try:
#         item_type = name_list['Type']
#     except:
#         item_type = ''
#     try:
#         brand = name_list['Brand']
#     except:
#         brand = ''
#     try:
#         model = name_list['Product Name']
#     except:
#         model = ''
#     try:
#         material = name_list['Exterior Material']
#     except:
#         material = ''
#     try:
#         line = name_list['Line']
#     except:
#         line = ''
#     try:
#         stamp = name_list['Date Code/Stamp']
#     except:
#         stamp = ''
#     try:
#         color = name_list['Exterior Color']
#     except:
#         color = ''
#     main_photo = response['Item']['PictureDetails']['PictureURL'][0]
#     other_photos = response['Item']['PictureDetails']['PictureURL'][1:]
#     try:
#         accessories = name_list['Accessories'].split(', ')
#     except:
#         accessories = ''
#     try:
#         condition = name_list['ConditionRank']
#     except:
#         condition = ''
#     try:
#         condition_comments = conditions[condition]
#     except:
#         condition_comments = ''
#     item_id = response['Item']['ItemID']
#     item_url = response['Item']['ListingDetails']['ViewItemURL']
#     try:
#         serial_number = name_list['Model Number']
#     except:
#         serial_number = ''
    
#     row = {
#         'title': title,
#         'price': price,
#         'currency': currency,
#         'item_type': item_type,
#         'brand': brand,
#         'model': model,
#         'material': material,
#         'line': line,
#         'stamp': stamp,
#         'color': color,
#         'serial_number': serial_number,
#         'main_photo': main_photo,
#         'other_photos': other_photos,
#         'accessories': accessories,
#         'condition': condition,
#         'condition_comments': condition_comments,
#         'item_id': item_id,
#         'item_url': item_url
#     }
#     rows.append(row)


# # data2 = [data.columns.values.tolist()]
# # data2.extend(data.values.tolist())
# # value_range_body = {"values": data2}

# # value_range_body = {
# #     "majorDimension":"COLUMNS",
# #     "values":
# #         [data2]
# #     }





